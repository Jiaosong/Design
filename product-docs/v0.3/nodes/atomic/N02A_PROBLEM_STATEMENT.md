...
| Product job | 把 observable situation 与 proposed solution 分开，形成可被设计处理的问题 |
...
用户提出的解决方案可以保留，但不能自动替代 problem definition。
...
- 只有 solution request → 系统提出 candidate problem，不写成 Current truth
