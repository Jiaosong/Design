# Timer R54 Blender 4.5 LTS / Cycles adapter
# STATUS: authored + syntax checked; runtime NOT RUN in current environment (bpy unavailable).
import hashlib
...
try:
    ...
    raise RuntimeError("Run this script inside Blender 4.5 LTS or a validated compatible Blender runtime") from exc
...
def main(glb_path, output_exr, profile="CMF"):
    ...
    bpy.context.scene.render.filepath = str(output_exr)
    bpy.ops.wm.save_as_mainfile(filepath=str(Path(output_exr).with_suffix(".blend")))
    bpy.ops.render.render(write_still=True)

# Execute main(...) from a validated Blender 4.5 LTS runtime.
