...
const PRESETS = {
  hero: {
    ...
    cards: [
      ...
      // Tall soft card creates the broad, continuous PC+ABS highlight instead of a clipped rim-only band.
...
export function buildStudioEnvironment(renderer, mode='hero') {
  ...
  // Mid-dark environment floor keeps satin metal body colour between bright reflection cards.
  capture.background = new THREE.Color(0x2b2824);
  preset.cards.forEach((def)=>reflectionCard(capture, def));
  // Narrow dark flag draws the product edge without turning housing / knob into near-black silhouettes.
...
export function addStudioLights(scene, mode='hero') {
  ...
  // ContactShadow is the single grounding system. Avoid stacked directional cast shadows / halos.
...
export function applyFocusLighting(lights, focus='body') {
  ...
  if(focus==='top' || focus==='diffuser'){
    // Low grazing key makes the shallow basin and edge thickness read without overexposing the centre.
    keyLight.intensity=4.15; keyLight.position.set(-2.4,1.55,2.45); keyLight.lookAt(0,0.24,0);
    fillLight.intensity=0.82; fillLight.position.set(3.1,1.25,2.0); fillLight.lookAt(0,0.23,0);
    rimLight.intensity=2.10; rimLight.position.set(-1.6,1.9,-3.0); rimLight.lookAt(0,0.24,0);
  } else if(focus==='control' || focus==='knob'){
    // Two-source metal sweep: one wide warm card response plus cool edge separation.
