...
const camera=new THREE.PerspectiveCamera(view.fov,4/3,.01,30);
camera.position.set(...view.cam);camera.lookAt(...view.target);
const renderer=new THREE.WebGLRenderer({antialias:true,powerPreference:'high-performance',preserveDrawingBuffer:true});
renderer.setPixelRatio(1);renderer.setSize(1200,900,false);configureColorPipeline(renderer,materialMode);
document.querySelector('#app').appendChild(renderer.domElement);
const studio=applyStudioScene(scene,renderer,shot);

const root=new THREE.Group();
root.name='MODEL_DERIVED_CALIBRATION_RIG';scene.add(root);
const groundOffset=.014;
const housingProfile=[
  [.505,.245],[.505,.290],[.530,.290],[.580,.260],[.590,.245],
  [.590,.020],[.560,.000],[.540,.020],[.540,.225],[.515,.242],[.505,.245]
].map(([r,y])=>new THREE.Vector2(r,y+groundOffset));
const housing=new THREE.Mesh(new THREE.LatheGeometry(housingProfile,192));
housing.name='01_Upper_Housing';root.add(housing);
...
diffuser.name='02_Formed_Diffuser';root.add(diffuser);

const bottomCover=new THREE.Mesh(new THREE.CylinderGeometry(.56,.56,.025,192),new THREE.MeshStandardMaterial());
bottomCover.name='18_Bottom_Cover';bottomCover.position.y=.0285;root.add(bottomCover);
const foot=new THREE.Mesh(new THREE.TorusGeometry(.5075,.0325,32,224),new THREE.MeshStandardMaterial());
foot.name='19_Silicone_Foot_Ring';foot.rotation.x=Math.PI/2;foot.scale.z=.23;foot.position.y=.0075;root.add(foot);
const knob=new THREE.Mesh(new THREE.CylinderGeometry(.08,.08,.08,128),new THREE.MeshStandardMaterial());
knob.name='17_Side_Knob';knob.rotation.x=Math.PI/2;knob.position.set(.18,.124,.602);root.add(knob);
const stateLight=new THREE.Mesh(new THREE.CylinderGeometry(.46,.46,.004,192),new THREE.MeshStandardMaterial());
stateLight.name='VISUALIZATION_State_Light';stateLight.position.y=.252;root.add(stateLight);

tuneProductMaterials(root,materialMode);
root.traverse(o=>{if(o.isMesh){o.layers.enable(1);o.castShadow=true;o.receiveShadow=true;}});

if(shot==='housing'){
  stateLight.visible=false;
  if('transmission' in diffuser.material){
    diffuser.material.transmission=.04;diffuser.material.roughness=.50;
    diffuser.material.color.setHex(0xe8e1d7);diffuser.material.needsUpdate=true;
  }
}
if(shot==='diffuser'){
...
new ContactShadow(renderer,scene,root,{
  resolution:shot==='shadow'?1024:768,
  opacity:shot==='shadow'?.62:.28,
  blur:shot==='shadow'?1.65:2.2
});
const post=createPostProcessing(renderer,scene,camera,materialMode);
...
document.querySelector('#label').textContent=`${shot.toUpperCase()} / ${window.__WEBGL_INFO.renderer}`;
