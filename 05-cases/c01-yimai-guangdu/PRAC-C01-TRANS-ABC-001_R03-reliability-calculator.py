...
def read(path):
    ...
    with open(path, encoding="utf-8-sig", newline="") as f:
        for line_no, row in enumerate(csv.DictReader(f), start=2):
            ...
            if not record_id:
                raise SystemExit(f"Missing record_id in {path} at CSV line {line_no}.")
            if record_id in rows:
                raise SystemExit(f"Duplicate record_id {record_id!r} in {path} at CSV line {line_no}.")
            rows[record_id] = row
    return rows
...
if only_a or only_b:
    raise SystemExit(f"Coder record_id sets differ. only_A={only_a} only_B={only_b}")
ids=sorted(A)
if not ids:
