...
const chapterLabels={
  hero:'项目定义',problem:'项目问题与机会',site:'场地与山水分析',culture:'地域文化与内容分析',audience:'人群与使用状态分析',behavior:'游程与行为分析',principles:'设计原理',methods:'设计方法',strategy:'总体策略与体验系统',journey:'路线、交通与服务设计',imprints:'十三印内容与互动系统',map:'探索地图',app:'数字陪伴系统',scene:'关键场景设计',physical:'实体、身体与感官设计',memory:'记忆与文化产品',detail:'设计深化与细节',technical:'技术与模型证明',process:'方案演化与专业判断',motion:'动效与跨媒介表达',return:'开放项、回程与结尾'
};
...
const chapterObs=new IntersectionObserver(entries=>{
  entries.forEach(e=>{
    if(e.isIntersecting){
      ...
      if(navState) navState.textContent=chapterLabels[id]||id;
    }
  })
...
$$('.nav-groups a').forEach(a=>a.addEventListener('click',e=>{const id=a.getAttribute('href')?.slice(1);if(id&&document.getElementById(id)){e.preventDefault();document.getElementById(id).scrollIntoView({behavior:'smooth'});}}));
...
if(field){
  imprints.forEach((it,i)=>{const b=document.createElement('button');b.className='imprint-btn';b.textContent=it.id.slice(1);b.dataset.index=i;const [x,y]=mapPos(i);b.style.left=x+'%';b.style.top=y+'%';field.appendChild(b);});
  const no=$('#imprintNo'),title=$('#imprintTitle'),mode=$('#imprintMode'),copy=$('#imprintCopy'),tags=$('#imprintTags');
  function selectImprint(i){const it=imprints[i];$$('.imprint-btn',field).forEach((b,j)=>b.classList.toggle('active',j===i));no.textContent=it.id;title.textContent=it.name;mode.textContent=it.mode;copy.textContent=it.copy;tags.innerHTML=it.tags.map(t=>`<span class="tag">${t}</span>`).join('');}
  field.addEventListener('click',e=>{const b=e.target.closest('.imprint-btn');if(b)selectImprint(+b.dataset.index)});selectImprint(5);
}
const map=$('#gameMap');
if(map){
  imprints.forEach((it,i)=>{const b=document.createElement('button');b.className='map-marker';b.textContent=it.id.slice(1);b.dataset.index=i;const [x,y]=mapPos(i);b.style.left=(8+x*.88)+'%';b.style.top=(18+y*.72)+'%';map.appendChild(b);});
  const title=$('#mapInfoTitle'),copy=$('#mapInfoCopy'),tags=$('#mapInfoTags');
  function selectMap(i){const it=imprints[i];$$('.map-marker',map).forEach((b,j)=>b.classList.toggle('active',j===i));title.textContent=`${it.id} · ${it.name}`;copy.textContent=it.copy;tags.innerHTML=it.tags.map(t=>`<span class="tag">${t}</span>`).join('');}
