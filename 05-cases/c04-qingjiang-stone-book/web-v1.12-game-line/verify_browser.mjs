...
try{
 ...
 const ctx=await browser.newContext({viewport:{width:1366,height:768},reducedMotion:'reduce'});const page=await ctx.newPage();await page.goto(baseUrl,{waitUntil:'domcontentloaded'});await settle(page);const reduced=await page.evaluate(()=>({mediaQueryMatches:matchMedia('(prefers-reduced-motion: reduce)').matches,scrollBehavior:getComputedStyle(document.documentElement).scrollBehavior,modeTransition:getComputedStyle(document.querySelector('.mode-card')).transitionDuration}));if(!reduced.mediaQueryMatches)fail('Reduced-motion preference not applied',{reduced});if(reduced.modeTransition!=='0s')fail('Motion transition remains under reduced motion',{reduced});report.reduced_motion=reduced;await ctx.close();
