...
const out = {schema:'C04_BROWSER_RUNTIME_VALIDATION_v3', base, cases:[], verdict:'PASS_BOUNDED', browserCarrier:'Chromium headless + ANGLE/SwiftShader WebGL', holds:['Browser runtime PASS is not Design KEEP.','FPS/ms/memory not asserted.','Visual fidelity to upstream source remains Presentation/Design review.']};
...
async function runCase(name,width,height,{reducedMotion='no-preference',forceError=false}={}) {
  ...
  const glb=[]; const consoleErrors=[]; const pageErrors=[]; const badResponses=[]; const requestFailures=[];
  page.on('response',async r=>{
    if(r.status()>=400) badResponses.push({url:r.url(),status:r.status()});
    if(r.url().endsWith('.glb')){let bytes=null;try{bytes=(await r.body()).length}catch{} glb.push({url:r.url(),status:r.status(),bytes})}
  });
  ...
  catch(e){ throw new Error(`${name}: harness boot failed; pageErrors=${JSON.stringify(pageErrors)} consoleErrors=${JSON.stringify(consoleErrors)} badResponses=${JSON.stringify(badResponses)} requestFailures=${JSON.stringify(requestFailures)}`); }
  ...
  catch(e){ const m=await page.evaluate(()=>window.__C04_METRICS); throw new Error(`${name}: model load failed; metrics=${JSON.stringify(m)} pageErrors=${JSON.stringify(pageErrors)} consoleErrors=${JSON.stringify(consoleErrors)} badResponses=${JSON.stringify(badResponses)} requestFailures=${JSON.stringify(requestFailures)} glb=${JSON.stringify(glb)}`); }
  ...
  const unexpectedBad=badResponses.filter(x=>!forceError || !x.url.endsWith('/__missing__.glb'));
  const pass=metrics.selectedLOD===expectedLOD && metrics.modelLoaded && metrics.interacted && metrics.hotspotSelected===true && successful.length===1 && pageErrors.length===0 && requestFailures.length===0 && unexpectedBad.length===0 && (!forceError || metrics.recovered===true) && (reducedMotion!=='reduce' || metrics.reducedMotion===true);
  out.cases.push({name,viewport:{width,height},reducedMotion,forceError,expectedLOD,metrics,glb,badResponses,requestFailures,consoleErrors,pageErrors,screenshot:shot,pass});
