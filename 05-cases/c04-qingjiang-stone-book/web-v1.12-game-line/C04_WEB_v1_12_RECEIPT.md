...
主作品阅读之外，固定右下角 `补充资料 / SUPPLEMENT` 入口承担第二阅读层。默认折叠，不增加主页面数量。7 组内容分别为原资产、设计创意、设计思路、技术应用路线、任务流程、创新点与技术难点、AI + 3D 创作过程。桌面固定右下角；移动端底部抽屉；ESC / 背景点击可关闭。源码含 `prefers-reduced-motion` 降级规则，但这只证明 source implementation exists，不证明 system-level reduced-motion runtime behavior 已验证。
...
- Historical route-package review state remains `EXECUTED / SELF-CHECKED / INDEPENDENT DESIGN REVIEW PENDING`; runtime materialization does not promote review state.
...
## HERO source resolution｜2026-08-27
...
The supplement deepens this reading without replacing or duplicating the main narrative. Original project assets remain the first source of visual evidence. Formal presentation text remains live HTML / SVG text.
...
- retired report-structure files absent;
- bottom-right supplement trigger present;
- 7 supplement categories present;
- every `assets/...` reference across HTML/CSS/JS resolves to an existing non-zero-byte file;
