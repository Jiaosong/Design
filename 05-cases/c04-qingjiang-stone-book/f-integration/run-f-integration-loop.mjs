...
for(const [lane,r] of Object.entries(state.lane_receipts)){
 ...
 if(r.receipt_type==='DIRECT'){
   check(`RECEIPT_${lane}_HASH`,Boolean(r.payload_hash_algo&&r.payload_hash),'DIRECT receipt requires payload hash');
   if(r.payload_hash_algo==='GIT_BLOB_SHA1')check(`RECEIPT_${lane}_BLOB_SHA`,/^[a-f0-9]{40}$/i.test(r.payload_hash||''),'Git blob SHA must be 40 hex');
 }
}
...
check('FR1_CURRENT_PR_ONLY',rr['F-R1']?.status==='CLOSED_IN_PR_CANDIDATE_MAIN_READBACK_PENDING','F-R1 remains PR-candidate until main readback');
...
for(const resolved of ['B_OWNER_CANONICAL','B_LANE_RECEIPT_OWNER_READBACK','R01_MODE_ALIGNMENT','D_STATE_VOCABULARY','D_EVIDENCE_STYLING','E_DEGRADED_OFFLINE_OWNER_CONTRACT','E_DIGITAL_OFF_COMPLETENESS'])check(`RESOLVED_BLOCKER_REMOVED_${resolved}`,!state.final_wording_blockers.includes(resolved),`${resolved} must not remain after direct receipt closure`);
for(const open of ['A_G1F','R02_EXPERT_EVIDENCE','R06_EXACT_SCIENCE_HERO_AND_FIELD_GEOMETRY','R07_FIELD_NAME_AUTHORITY','R13_IMAGE_ASSET_AND_FIELD_SAFETY','F_MAIN_READBACK'])check(`OPEN_BLOCKER_${open}`,state.final_wording_blockers.includes(open),`${open} remains promotion blocker`);
