#!/usr/bin/env python3
"""Discover image assets embedded in a project-approved official/operator page.

This tool extracts and downloads page images in document order and creates a
contact sheet for human node identification. It deliberately does NOT infer a
Node PASS from DOM adjacency or article text.
"""
...
def request_bytes(url: str, accept: str) -> bytes:
    req = urllib.request.Request(
        url,
        headers={
            "User-Agent": "Mozilla/5.0 OLEANDER-C04-Official-Page-Media/1.0",
            "Accept": accept,
            "Referer": PAGE_URL,
        },
    )
...
def make_sheet(records: list[ImageRecord]) -> None:
    ...
    for i, record in enumerate(records):
        ...
        label = f"#{record.order:02d}  {record.width}x{record.height}\n{record.filename[:42]}"
...
def main() -> int:
    ...
    payload = {
        "schema": "oleander.c04.official-page-media-discovery.v0.1",
        "page_url": PAGE_URL,
        "policy": {
