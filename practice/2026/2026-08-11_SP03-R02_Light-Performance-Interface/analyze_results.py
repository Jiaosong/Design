...
for scheme in ['A','B']:
    rows=all_sensor[(scheme,'OVC')]
    d={(float(r['x_m']),float(r['y_m'])):float(r['lux']) for r in rows}
    Z=np.array([[d[(x,y)] for x in xs] for y in ys])
    fig,ax=plt.subplots(figsize=(10,4.5))
    im=ax.imshow(Z,origin='lower',extent=[min(xs),max(xs),min(ys),max(ys)],aspect='auto')
    ax.set_title(f'SP03-R02 | Scheme {scheme} | OVC | workplane illuminance [lux]')
...
print(json.dumps({'gate':gate,'aggregate':aggregate},indent=2))
