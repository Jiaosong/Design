...
def main():
    ...
    report={'schema':'oleander.modeling-worker.v0.13.g1.r2.blender-report','model':MODEL,'status':status,'job_state':'EXECUTED','design_state':'EXPLORE','authority_state':'WORKING_SOURCE','candidate_promotion':'NOT_RUN','blender_version':bpy.app.version_string,'render_engine':scene.render.engine,'diagnostic_exposure':scene.view_settings.exposure,'checks':checks,'machine_baseline':base_report,'machine_revision':rev_report,'source_objects':[o.name for o in src],'derived_objects':[base.name,rev.name],'camera_jobs':{'HERO_CAM':{'lens_mm':85,'location':[.34,-.34,.25]},'CMF_CAM':{'lens_mm':110,'location':[.095,0,1.20]},'INSPECTION_CAM':{'lens_mm':135,'location':[.95,0,.075]}},'rigs':rigmap,'diagnostics':rendered,'master_exr':master,'blend':blend.name,'source_sha256':sha(a.source),'correction_sha256':sha(a.correction),'contract_sha256':sha(a.contract),'boundary':contract['boundary']}
