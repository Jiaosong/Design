...
This is a source-locked A/B diagnostic. It does not revise either candidate.
Both candidates are rendered with the same corrected visible wheel package:
- X/Z OD = 0.700 m;
- centers = FX/RX, +/-WY, WZ;
- existing Y tire thickness retained;
- same derived-only near-side wheelhouse liners;
- same nine-view comparison matrix.
"""
from __future__ import annotations
import argparse,importlib.util,bpy,bmesh,json,math,sys
...
def normalize_wheels(b,M,target_od=.700):
    ...
    for o in [x for x in bpy.context.scene.objects if x.type=='MESH' and x.name.startswith('WHEEL_')]:
        before=raw_wb(o);x=b.FX if o.name.startswith('WHEEL_F') else b.RX;y=b.WY if '_L_' in o.name else -b.WY;z=b.WZ;target=[float(x),float(y),float(z)];cx,cy,cz=before['center'];dx,dy,dz=before['dimensions'];fx=target_od/dx;fz=target_od/dz;inv=o.matrix_world.inverted()
        for v in o.data.vertices:
            p=o.matrix_world@v.co;p.x=target[0]+(p.x-cx)*fx;p.y=target[1]+(p.y-cy);p.z=target[2]+(p.z-cz)*fz;v.co=inv@p
        o.data.update();bpy.context.view_layer.update();after=eval_wb(o);records.append({'name':o.name,'target_center':target,'before_raw':before,'after_evaluated':after,'y_thickness_target':dy})
...
def main():
    ...
    views=[
      ('SIDE_SILHOUETTE_HP',(0,-8.8,1.14),(0,0,.64),85,True,5.25,'BROAD','sil','near'),
      ('PACKAGE_SIDE_NEARSIDE',(0,-8.8,1.14),(0,0,.64),85,True,5.25,'BROAD','normal','near'),
      ('HERO_FRONT_NEARSIDE',(6.2,-7.0,2.75),(.05,0,.66),78,False,5,'BROAD','normal','near'),
      ('HERO_REAR_NEARSIDE',(-6.0,-6.8,2.65),(-.10,0,.66),78,False,5,'BROAD','normal','near'),
      ('CLAY_STRIP_NEARSIDE',(6.2,-7.0,2.75),(.05,0,.66),78,False,5,'STRIP','normal','near'),
      ('CLAY_GRAZING_NEARSIDE',(6.2,-7.0,2.75),(.05,0,.66),78,False,5,'GRAZING','normal','near'),
      ('FRONT_ARCH_ISOLATED',(2.65,-3.4,.95),(b.FX,-b.WY,.48),100,False,5,'STRIP','normal','front'),
      ('REAR_ARCH_ISOLATED',(-2.55,-3.4,.95),(b.RX,-b.WY,.48),100,False,5,'STRIP','normal','rear'),
    ...
    for lab,loc,tgt,lens,ortho,scale,rig,mode,scope in views:
        ...
        for wh in liners:
            if mode=='wire' or mode=='sil':wh.hide_render=True
    ...
    bpy.context.view_layer.material_override=None;source.hide_render=False;h2=r20.shape_hash(source);mods=[m.type for m in source.modifiers]
    blend=out/f'{model}.blend';bpy.ops.wm.save_as_mainfile(filepath=str(blend))
    checks={'source_shape_hash_locked':h0==h1==h2,'source_island_count_one':islands==1,'source_ngon_zero':stats['ngon']==0,'termination_triangles_four':stats['tri']==4,'source_no_boolean':'BOOLEAN' not in mods,'source_no_subd':'SUBSURF' not in mods,'wheel_hp_package_exact':wheel_ok,'wheelhouse_count_four':len(liners)==4,'render_matrix':len(R)==9}
    q={'schema':'oleander.auto.v0.11.hp-rebaseline.qa','variant':a.variant,'model':model,'status':'MACHINE_PASS_VISUAL_REVIEW_REQUIRED' if all(checks.values()) else 'MACHINE_FAIL','source_hash':h0,'topology':stats,'source_island_count':islands,'source_boundary_reuse_or_endpoint_count':reuse,'wheel_package':wheel_records,'checks':checks,'renders':R,'boundary':'Source candidate locked. A/B changes visible wheel implementation to exact M1 package and uses identical derived wheelhouse/near-side evidence only. No design variable is revised.'};(out/'AB_QA.json').write_text(json.dumps(q,ensure_ascii=False,indent=2)+'\n')
    c={'contract_version':'v0.2','spec_patch':'v0.2.1','job_id':f'SYS-MODELING-WORKER-AUTO-M1M5-v0.11-{a.variant}-HP-REBASELINE','modeling_stage':'M5','design_state':'REVISE','authority_state':'DIAGNOSTIC_ONLY','source_authority':{'state':'LOCKED_FOR_AB_REBASELINE','artifact_hash':h0},'revision':{'source_change':False,'wheel_package_change':'deterministic implementation correction to exact M1 hard points','comparison_peer':'R28A' if a.variant=='R25' else 'R25'},'qa':{'project':['compare package, arch containment, shoulder/fender continuity, strip/grazing curvature and wire topology under identical corrected wheel evidence','M6/M7/M8 remains blocked']}};(out/'MODELING_CONTRACT.json').write_text(json.dumps(c,ensure_ascii=False,indent=2)+'\n')
