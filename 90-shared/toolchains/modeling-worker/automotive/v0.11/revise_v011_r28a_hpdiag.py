...
def wb(o):
    ...
    return {'min':mn,'max':mx,'dimensions':[mx[i]-mn[i] for i in range(3)]}
...
def fixed_wheels(M):
    ...
    for o in [x for x in bpy.context.scene.objects if x.type=='MESH' and x.name.startswith('WHEEL_')]:
        ...
        o.data.update();after=wb(o);o['OLEANDER_HP_WHEEL_OD_M']=TARGET_OD;o['OLEANDER_HP_NORMALIZATION']='WORLD_XZ_ENVELOPE_ONLY'
...
def main():
    ...
    a=diag.b.parse();out=Path(a.out).resolve();data={'schema':'oleander.auto.v0.11.wheel-hp-normalization','target_od_m':TARGET_OD,'source_change':False,'wheel_mesh_count':len(FIX_RECORD),'records':FIX_RECORD}
    (out/'WHEEL_HP_NORMALIZATION.json').write_text(json.dumps(data,ensure_ascii=False,indent=2)+'\n')
    qp=out/'AUTOMOTIVE_V011_QA.json';q=json.loads(qp.read_text());q['schema']='oleander.auto.v0.11.r28a.hpdiag.qa';q['checks']['wheel_hp_od_normalized']=len(FIX_RECORD)==4 and all(abs(r['after']['dimensions'][0]-TARGET_OD)<1e-5 and abs(r['after']['dimensions'][2]-TARGET_OD)<1e-5 for r in FIX_RECORD);q['boundary']='R28A Source locked. Four visible wheel meshes normalized in world X/Z to locked 0.70 m OD before diagnostic rendering; wheel Y thickness and centers unchanged.';q['status']='MACHINE_PASS_VISUAL_REVIEW_REQUIRED' if all(q['checks'].values()) else 'MACHINE_FAIL';qp.write_text(json.dumps(q,ensure_ascii=False,indent=2)+'\n')
    cp=out/'MODELING_CONTRACT.json';c=json.loads(cp.read_text());c['job_id']='SYS-MODELING-WORKER-AUTO-M1M5-v0.11-R28A-HPDIAG';c['revision']={'revision_id':'R28A-WHEEL-HP-DIAGNOSTIC','source_change':False,'wheel_display_change':'normalize WHEEL_* world x/z envelope to 0.70 m locked OD','design_variable_change':False};c['qa']['construction'].append('R28A Source unchanged; wheel visualization corrected to locked M1 hard point before Human M5');cp.write_text(json.dumps(c,ensure_ascii=False,indent=2)+'\n')
