...
No design geometry change. Uses in-place material-slot replacement and relative-red pixel proof.
"""
...
def mask(out,label,loc,target,ortho,scale,lens):
    ...
    white=emission_mat('MAT_MASK_BODY',(1,1,1,1));red=emission_mat('MAT_MASK_GLASS',(1,0,0,1));assert len(dup.data.materials)>=2
    ...
    for i in range(0,len(px),4):
        r,g,b,al=px[i:i+4]
        # Blender image pixels are scene-linear. Use relative red dominance instead of a display-space absolute .75 threshold.
        if r>.40 and r>2.5*g and r>2.5*b:rp+=1
        if r>.55 and g>.55 and b>.55:wp+=1
