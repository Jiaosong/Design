...
export async function syncPage(env: Env, message: IngestMessage): Promise<{ status: string; page_id: string; chunks?: number }> {
  ...
  const contentHash = await sha256Hex(
    stableStringify({
      ...
      metadata: {
        ...
        object_plane: page.objectPlane ?? "KNOWLEDGE",
        evidence_grade: page.evidenceGrade ?? null,
        freshness_state: page.freshnessState ?? null,
        authority_class: page.authorityClass ?? null,
        information_role: page.informationRole ?? null,
        secondary_semantics: page.secondarySemantics ?? [],
        topic_ids: page.topicIds ?? [],
        claim_ids: page.claimIds ?? [],
        evidence_ids: page.evidenceIds ?? [],
        capability_links: page.capabilityLinks ?? [],
        function_links: page.functionLinks ?? [],
        information_entity_links: page.informationEntityLinks ?? [],
        interface_links: page.interfaceLinks ?? [],
        work_medium: page.workMedium ?? [],
        design_scale: page.designScale ?? [],
        artifact_type: page.artifactType ?? null,
        symptom_tags: page.symptomTags ?? [],
        affected_relation_tags: page.affectedRelationTags ?? [],
        tool_capability: page.toolCapability ?? [],
        readback_method: page.readbackMethod ?? [],
  ...
  for (let i = 0; i < chunks.length; i += 1) {
    ...
    const metadata = {
      ...
      content_level: page.contentLevel ?? "",
      framework_type: page.frameworkType ?? "",
      retrieval_space: page.retrievalSpace ?? "",
      search_eligibility: page.searchEligibility ?? "",
      governance_state: page.governanceState ?? "",
      trust_state: page.trustState ?? "",
      freshness_state: page.freshnessState ?? "",
      evidence_grade: page.evidenceGrade ?? "",
      authority_class: page.authorityClass ?? "",
      project_id: page.primaryProjectIds[0] ?? "",
      domain_id: page.primaryDomainIds[0] ?? "",
