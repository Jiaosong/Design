...
Post-sync knowledge governance uses the bearer-protected `/v1/governance-page` route. `GET` reads the live Notion page plus markdown; `POST` is intentionally allowlisted to `canonical_id`, `retrieval_space`, and `search_eligibility`, derives the correct Notion property payload from the live page schema, then immediately runs the normal `syncPage` path and returns before/after readback. It is not a bulk mutation API: operators must complete content review and a shadow decision before each write. Permanent deletion, arbitrary property writes, and relation rewrites are not exposed here.
