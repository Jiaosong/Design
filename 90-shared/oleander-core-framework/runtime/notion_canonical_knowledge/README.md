...
Post-sync knowledge governance uses the bearer-protected `/v1/governance-page` route. `GET` reads the live Notion page plus markdown. `POST` keeps the existing low-level allowlist (`canonical_id`, `retrieval_space`, `search_eligibility`, `governance_state`, `relation_state`) for explicit operator recovery, and also accepts a bounded Reader `intent` (`REVIEW`, `HOLD`, `BLOCK_SEARCH`, `ARCHIVE`). Intent mode requires a shadow-decision rationale plus exact expected Canonical ID and Notion `last_edited_time`; the Worker re-reads live Notion, resolves the scalar patch itself, rejects no-op/stale requests, and for CURRENT archive requires exactly one live replacement in the same Notes data source with `CURRENT / ACTIVE / VALID` state. The route then runs the normal `syncPage` path and returns the authoritative resolved delta plus before/after readback. If the Notion patch succeeds but derivative sync fails, the response explicitly reports `notion_write_applied=true` and `governance_sync_failed_after_notion_write` instead of collapsing the transaction into an ambiguous generic failure. It is not a bulk mutation API. Permanent deletion, arbitrary property writes, Trust promotion, CURRENT promotion, and relation-target rewrites are not exposed here.
...
Human reading is separated from governance storage. `POST /v1/reader-layer` creates one idempotent root-level `知识阅读台｜Knowledge Reader` backed by linked views of the existing Notes data source: Core Knowledge (`L4/L5 + ACTIVE + VALID`), Methods (`METHOD + ACTIVE + VALID`), Evidence (`L6 + ACTIVE + VALID`), Practice (`L7 + ACTIVE`), and History/Governance (`PROVENANCE` or `LEGACY/ARCHIVED/HOLD`). The reader layer does not duplicate the database and does not alter authority. `POST /v1/reader-layer/visualize` upgrades that same reader into a human-first visual surface using the Notion Views API: a native dashboard becomes the first database tab, with Core/Evidence donut charts plus Current/Methods/Practice gallery widgets; existing gallery/chart/list views remain available as drill-down and fallback views. The endpoint is idempotent, records its live IDs/readback in D1 `reader_visual_v1`, and only rewrites the reader-intro section before the preserved linked-database blocks.

`GET /v1/reader-page/:page_id` remains detail-only/lazy and now appends `frameworkReadback` from live Notion without enlarging the corpus snapshot or creating new storage. The readback hydrates the existing `主领域` / `关联领域` relations to Current Domain-registry identities, reads `Canonical Parent｜层级上位` / `Canonical Children｜层级子级` as dedicated hierarchy relations, and exposes the live `方法家族` values used by the existing resolver contract. Long Notion relations are paged through the page-property endpoint instead of being silently truncated. A shared 8-second Reader-detail deadline starts before the D1 detail read and is passed through relation pagination, Domain-registry inventory, retries, response-body parsing, the end-of-read root revision check, and any optional D1 execution-context read; the endpoint fails boundedly instead of consuming the Reader client's 12-second timeout, while schema/cursor/revision gaps yield explicit `PARTIAL` / `STALE_DURING_READBACK` state rather than being presented as an authoritative empty relation.

Required native output remains an execution-context input rather than a knowledge-object property. Without an explicit `task_id`, execution-owner output remains `NOT_HYDRATED` and `CURRENT_EXECUTION_CONTEXT_NOT_BOUND`. When a caller supplies `task_id`, the Worker may bind owner/native-output readback only from an existing `OLEANDER_EXECUTION_RECEIPT_v1.0` context that was projected through `/v1/execution-live-status`, whose receipt `authority.canonical_ids` contains the exact Reader object's Canonical ID. The highest checkpoint sequence wins; stale, malformed, mismatched, missing, or conflicting same-sequence contexts fail closed and do not affect the live Notion framework readback. The Worker never reconstructs `OLEANDER_NOTION_TO_GITHUB_EXECUTION_OWNER_MAP_v1.0` locally, never infers an owner from Domain/Method/title, and never treats D1 as owner authority: D1 is only the task-scoped carrier of the already-resolved Execution Receipt result.
