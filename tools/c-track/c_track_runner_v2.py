...
def c01():
 ...
 for q in ["RAW","MAP","POWER","ABS","HARD","SOFT","RING"]:
  ...
  if q=="MAP":
   ...
   sp=t.nodes.new("ShaderNodeSeparateXYZ");t.links.new(v,sp.inputs["Vector"])
   sx=mm(t,"SUBTRACT",sp.outputs["X"],.5).outputs[0];sy=mm(t,"SUBTRACT",sp.outputs["Y"],.5).outputs[0]
   cb=t.nodes.new("ShaderNodeCombineXYZ");t.links.new(sx,cb.inputs["X"]);t.links.new(sy,cb.inputs["Y"])
   l=t.nodes.new("ShaderNodeVectorMath");l.operation="LENGTH";t.links.new(cb.outputs["Vector"],l.inputs[0])
   s=mm(t,"LESS_THAN",mm(t,"ABSOLUTE",mm(t,"SUBTRACT",l.outputs["Value"],.32).outputs[0]).outputs[0],.045).outputs[0]
...
def render(mid,items,O,a):
 ...
 for i,(lab,m) in enumerate(items):
  x=(i%cols-(cols-1)/2)*2.25;y=((rows-1)/2-i//cols)*2.25
  if mid=="C07":
   bpy.ops.mesh.primitive_cube_add(size=1.65,location=(x,y,.825))
  else:
   bpy.ops.mesh.primitive_plane_add(size=1.8,location=(x,y,0))
  bpy.context.object.name=mid+"_"+lab;bpy.context.object.data.materials.append(m)
 c=bpy.data.cameras.new("CAM");c.type="ORTHO";o=bpy.data.objects.new("CAM",c);bpy.context.collection.objects.link(o)
 if mid=="C07":
  o.location=(8,-11,10);o.rotation_euler=(Vector((0,0,.4))-o.location).to_track_quat("-Z","Y").to_euler();c.ortho_scale=max(cols*2.25,rows*2.25*1.5)*1.28
 else:
  o.location=(0,0,10);o.rotation_euler=(0,0,0);c.ortho_scale=max(cols*2.25,rows*2.25*1.5)*1.08
 bpy.context.scene.camera=o;s=bpy.context.scene;s.render.engine="CYCLES";s.cycles.samples=a.samples;s.render.resolution_x=1800;s.render.resolution_y=1200;s.render.resolution_percentage=100;s.render.image_settings.file_format="PNG";s.world.color=(0,0,0);O.mkdir(parents=True,exist_ok=True);(O/"renders").mkdir(exist_ok=True);(O/"blends").mkdir(exist_ok=True);s.render.filepath=str(O/"renders"/(mid+".png"));bpy.ops.wm.save_as_mainfile(filepath=str(O/"blends"/(mid+".blend")));bpy.ops.render.render(write_still=True);return {"module":mid,"labels":[x[0] for x in items],"status":"RENDERED_POST_REVIEW_REQUIRED","test_geometry":"CUBE_3D" if mid=="C07" else "PLANE_2D"}
def main():
