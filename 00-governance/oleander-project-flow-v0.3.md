...
For professional work, `PROFESSIONAL_EXECUTION_CAPABILITY_PLAN` is the machine-readable route from required capability/native output to a real Current owner or validated specialist binding and observed execution surface. `EXECUTABLE` means the route is currently actionable at the declared claim ceiling; it does not mean the native artifact has been produced, read back or professionally accepted. `CANDIDATE_ONLY`, `FIELD_ONLY`, `USER_MANUAL_BOUND` or `CAPABILITY_HOLD` remain visible blockers whenever their output is in claim and no valid closure route is bound.
