...
- 31 asset-level rows
- 23 unique chains
- 8 confirmed `PERSISTENCE FAIL` chains
- 1 current `PERSISTENCE AT RISK` chain
- 7 healthy / durable-recoverable control chains
...
3. `SP01-R02-GIS` — full QGIS runtime artifact is currently recoverable only from an expiring GitHub Actions artifact. Audit re-download SHA matched `68ce760293057b3595856b8c935ed3cb51c4c512ea6f9a6ea5eb5614b225d00c`; unzip PASS. Actions expiry: `2026-09-10T03:15:45Z`. Status: `PERSISTENCE FAIL / RECOVERABLE NOW`.
4. `SP02-RELATIONAL-FIELD` — real Rhino `.3dm` plus two historical production ZIPs are not present in the canonical Drive folder and are not currently materializable from File Library.
5. `SP03-ARCH-GEN` — real `OLEANDER_Architecture_Generation_Protocol_01.3dm` was historically read-back verified but the current bytes are unavailable.
6. `B04-CROSS-MEDIA` — Notion/migration record and report survive; `OLEANDER_Cross_Media_Validation_2026-08-06.zip` does not.
7. `IP03-VISUAL-HIERARCHY` — inner evidence files partially survive in Drive, but `OLEANDER_Visual_Grid_01_Blocker_Resolution_v0.2_2026-08-07.zip` does not.
8. `SYNC-REPAIR-LEGACY` — `OLEANDER_GitHub_Notion同步修复包_2026-08-06.zip` has a recorded SHA but no current durable binary. It is legacy WORKING material, not current governance authority.
...
This is the reference pattern for retroactive PAP closure. Persistence PASS does not imply a real `.gh/.ghx` solve; that runtime gate remains separate.
...
Preserve `SP01-R02-GIS` exact Actions artifact into a qualified durable store before `2026-09-10` and verify the same SHA after Drive re-download.
...
- `PERSISTENCE FAIL / RECOVERABLE NOW`

Remediation order is fixed as:

`P0 rescue → P1 current-authority recovery → P2 historical recovery → P3 receipt/checksum backfill`
