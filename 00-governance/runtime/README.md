...
The persisted CoS binding keeps **Chat as the default Human interaction surface** while allowing workstation capabilities to execute behind the conversation through CoS. Beginning with bootstrap v1.6, that CoS binding is intentionally stable and points to `OLEANDER_CHAT_ENTRY_RUNTIME.md`; the current entry is reread on each OLEANDER-scoped turn. Capability routes therefore remain hot-updatable repository/runtime content instead of being frozen inside the CoS process configuration. Conversation surface, execution surface and mutation permission remain separate. The first exercised local capability route is `BAIDU_STORAGE`: Chat may hand read work to the candidate `codesign_chat_cos_bridge_v0_1.py`, which invokes installed `oleander-baidu-storage@oleander-personal` v0.1.1 over stdio and returns provider readback to the same Chat. A local route grants no authority; writes still require the existing owner-native mutation/authorization guard. If the local surface is unavailable, use the existing continuity execution-intent carrier and report `PENDING_LOCAL_EXECUTION` rather than creating a second queue or asking the Human to reopen the task in Codex. Ordinary route/runtime updates do not rewrite the CoS config or require a restart; only a bootstrap-mechanism change does. CoS itself remains independently updatable: OLEANDER does not own or pin the CoS application version, updater or installer. `OLEANDER_COS_UPDATE_COMPATIBILITY_CONTRACT_v0.1.md/.json` plus `validate_cos_update_compatibility.py` define the post-update compatibility probe. Drift HOLDs only the affected OLEANDER integration; it never blocks the CoS update or requires downgrade. The validator is read-only by default; `--repair-bootstrap` is an explicit bounded rebind through the existing backup-producing binder when the config schema is still compatible.
