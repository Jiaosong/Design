...
| CD-02 | REAL_C01_TRIAL | materially distinct editable mechanisms under one open design question |
| CD-03 | CONTRACT + REAL HUMAN OPEN | negative steering remains causal hypothesis / no durable taste; real Human round remains partial until performed |
| CD-04 | REAL HUMAN OPEN | MIX requires actual Human parent-referenced steer; no fabricated second round |
...
| CD-08 | CONTRACT + REAL HUMAN OPEN | designer-development pattern exists without competence profile; real interaction remains partial until performed |
...
| CD-10 | REAL_CROSS_DOMAIN_PRE_STEER | shared interaction model with truthful domain process/native/readback boundaries; Human-steered closure remains partial |
...
Session Kernel v0.2 has a separate deterministic reference harness in `validate_codesign_session_kernel_v0_2.py`. The current 98-case fixture set covers interaction-axis separation, compound/conflicting actions, exact/deictic/colliding-alias referent binding, Chinese/English route-only `DESIGN_DECISION / DESIGN_KEEP / PROMOTION_DECISION`, typed/revisioned active-object steering, pending-decision-scoped DEFER, executable branch transitions, artifact-revision/content-hash comparison proof, owner-native hash-bound decision-rights proof, invariant-specific readback proof where the readback itself must enumerate verified invariants, fresh canonical-carrier mutation guards including fail-closed missing decision-rights/user-constraint projections, computed Human-stop/defer dependency behavior, designer-support fade, OPEN-domain process boundaries, progressive-disclosure context loading and owner-preserving file placement. These machine cases are kernel-behavior checks, not substitutes for the real-Human partials above.
