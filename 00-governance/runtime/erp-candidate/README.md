# OLEANDER Enterprise Orchestration Candidate v0.3

Status: **EV2 EVAL READY / NON-AUTHORITATIVE / READ-ONLY CANDIDATE**

This package tests whether OLEANDER can expose a unified enterprise orchestration/read model across **ERP + PLM + MES + BPM + QMS + MBSE + Knowledge Graph + Agent Runtime** without creating a second OLEANDER architecture, a second project-state database, a universal state machine, a persistent event ledger, or a distributed transaction manager.
...
The eight modules are concerns/views, not new authority layers:
...
5. **No hidden promotion.** `JOB/AGENT SUCCEEDED`, `PLM RELEASED`, `MES COMPLETE`, `BPM COMPLETED`, `QMS PASS`, or `MBSE VERIFICATION PASS` never imply `DESIGN KEEP`, `PROFESSIONAL PASS`, `VALIDATION PASS` or `PROJECT PROMOTION`.
...
## 4. Active machine files

- `OLEANDER_ENTERPRISE_ORCHESTRATION_PROJECTION_v0.3.schema.json` ? active v0.3 envelope schema.
- `example_enterprise_projection_v0.3.json` ? rich cross-module fixture.
...
- `build_erp_projection.py` ? conservative Master Runtime -> v0.3 projection builder.
- `validate_erp_candidate.py` ? structural, authority and cross-module invariant validator.
- `OLEANDER_EVOLUTION_CANDIDATE_ERP_ORCHESTRATION_STAGE1_20260921.json` ? controlled-evolution candidate record.
- `ERP_STAGE1_EVAL_RECEIPT_v0.3_20260922.json` ? v0.3 structural/reference evaluation receipt; real cases and independent review remain open.
- `ENTERPRISE_CANDIDATE_MANIFEST_v0.3.json` ? active-file canonical hash manifest for exact candidate readback; self-excluded.

v0.1/v0.2 schema/fixtures/eval output remain retained as candidate provenance and are **not** the active model.

## 5. What the builder deliberately does not invent
