...
10. an unmerged orphan ref may be classified `SAFE_DELETE_PATCH_EQUIVALENT_UNMERGED_ORPHAN` only when it is dependency-free, its `origin/main..origin/<branch>` range contains no merge commits, `git cherry origin/main origin/<branch>` returns at least one commit, every result is `-` (patch-equivalent already absorbed by main), and the cherry-result count equals the unique commit count. Any `+` result, merge commit, empty/ambiguous result or count mismatch blocks automatic deletion.
11. an open PR must not permanently retain an already-merged historical base when it can be safely retargeted. Retarget `base → main` only when the old base is already contained in `main`, the PR head still has real unabsorbed commits, the exact head ref is unchanged, and a hypothetical `main + head` merge tree is clean. After readback confirms the PR still carries the intended head delta, the old merged base ref becomes a normal cleanup candidate. Any semantic ambiguity, conflict or dependency requires `HOLD` and base retention.
12. `main` is a PR-only integration surface. Human, agent and admin-authored Current mutations must use `work branch → commit → PR → applicable CI → merge → main readback`; direct pushes, force pushes and branch deletion are forbidden by default. A temporary emergency bypass is permitted only as an explicit bounded governance incident with pre-state capture, reason, affected SHA/range, postcondition readback and immediate protection restoration.

Content-level disposition for an unmerged orphan branch uses exactly five states:

- `ACTIVE` — the ref still owns a live execution/validation lineage or is directly consumed by Current runtime/workflow state. Retain it; cleanup is blocked until that dependency is closed or migrated.
- `REVIEW` — the branch contains unique or semantically ambiguous content for which Current authority does not yet prove keep/reject/absorb. Retain it; neither merge nor delete is implied.
- `REJECT` — the content has an explicit design/governance rejection, withdrawn scope, `REVISE_RETURNED`, `NO_PROMOTION`, or equivalent negative disposition. It must not be promoted. The ref may be removed only after durable provenance is preserved and dependency checks are clear.
- `ABSORB` — the branch contains bounded useful content that belongs in an existing Current/Support/Process carrier. Absorb selectively without upgrading its truth/status claims; validate and read back the destination, then reclassify the old ref `SUPERSEDED` before cleanup. Whole-branch merge is not the default.
- `SUPERSEDED` — a later authority/successor, merged PR, absorbed implementation, or explicit provenance state has replaced the branch as a live frontier. The ref is cleanup-eligible only after durable provenance and dependency checks; `SUPERSEDED` alone is not deletion authority.
