...
Strengthen the existing Candidate with these exact object contracts when relevant:
...
### Hard attacks
- `TRACE COMPLETE ≠ REQUIREMENT VALID` when source intent/testability is weak;
- `VERIFICATION ≠ VALIDATION`; requirement conformance cannot close intended-use evidence;
...
Every V&V result must bind exact configuration identity. A material requirement/interface/configuration change reopens the affected verification, integration and validation scope rather than merely editing the latest document.
