...
#### B｜Minimum visible section responsibilities

Professional-domain wording may vary, but an in-claim architecture stage body must visibly resolve, combine or mark `NOT_APPLICABLE_WITH_REASON` for the following responsibilities:

1. `Stage Question / Scope`;
2. `Current Condition / Problem`;
3. `Authority / Knowledge Inputs` — current project authority plus task-scoped canonical knowledge/evidence refs and their `OE* / applicability / claim ceiling / does_not_prove` boundaries;
4. `Architectural Criteria / Intent`;
5. `Development / Comparison / Mechanism`;
6. `Professional Interfaces` when materially coupled;
7. `Native Output / Source of Truth`;
8. `Actual Readback / Finding`;
9. `Failure / OPEN / does-not-prove`;
...
- for each water-exposed architectural assembly in claim—roof/terrace/balcony, entrance/threshold, wet room, plant enclosure or comparable condition—trace `water source → falls/high-low points → collection → primary waterproofing/containment → penetration/junction → outlet → overflow/secondary path → final receptor + maintenance access` on the actual section/detail;
- coordinate drain/flange/frame/kerb/upstand/recess and finish build-ups so finished geometry does not reverse falls, bury the waterproofing termination, create an inaccessible trap or force a late threshold/accessibility compromise;
- at substrate changes, movement joints, door/window sills and service/structural penetrations, show how the water-control layer terminates, transitions and remains inspectable/repairable while accommodating owner-supplied movement/tolerance;
- read one credible adverse water state where material—blocked primary outlet, wind-driven exposure, saturated finish build-up, construction-phase incomplete enclosure or comparable failure—so the geometry exposes where water goes when the preferred path fails;
- when the controlling owner requires flood/spray/water testing or a mockup, bind the result to the exact substrate/detail/material/revision; a passed different configuration is not waterproofing evidence. Envelope/Waterproofing/MEP/Civil retain capacity, material, system criteria and formal performance approval; Architecture closes geometry, continuity and spatial consequence only;
...
Dimensional coordination must also close the **datum → tolerance stack → absorption → as-built** chain at selection-critical interfaces:

- establish the controlling setting-out datum/reference plane and distinguish design nominal geometry from required finished/use geometry, owner-supplied fabrication/installation tolerance and predicted movement;
- trace `structure/substrate → support/frame → lining/build-up → finish → final door/opening/casework/equipment/route clearance`; do not compare discipline nominals referenced to different faces/datums as if they were the same dimension;
- identify where variation is deliberately absorbed—adjustable fixing, joint/reveal, packing zone, trim, service zone, sacrificial finish or another designed allowance. Field trimming that consumes accessibility, fire, acoustic, waterproofing, maintenance or usable-space reserve is not a valid tolerance strategy;
- attack the credible combined tolerance/movement state at critical doors, thresholds, facade openings, ceiling/service zones, casework/equipment interfaces and repeated alignments; consume actual tolerance requirements from Structure/Envelope/MEP/Interior/product owners rather than inventing generic values;
- where tightly fitted/repeated work controls the claim, bind survey/as-built or sample readback to the relevant datum and exact detail revision. Coordinated model geometry remains nominal evidence only. Architecture owns dimensional/spatial consequence; specialist tolerance/performance/compliance remains owner-native.
...
| `ADD-17` | independent readback finds the current plan/section/model operationally coherent at the claimed ceiling; strongest success and strongest consequential contradiction are both recorded | bounded Architecture verdict + exact reviewed artifact/revision + failed/reopen relations to DQ, R-F and Master compilation | any material reviewed geometry, operational state, interface, authority or native-artifact revision changes |
...
- `trigger_reason`;
- `project_stage`;
- `stage_body_records` for every materially executed/in-claim ADD stage, including specific title, visible semantic section coverage, task-scoped knowledge-mount refs, native artifact refs, readback refs, claim ceiling, OPEN items and reopen triggers;
- `jurisdiction_state`;
...
For final Architecture `PASS`, every in-claim stage-body responsibility must be `PRESENT`, intentionally `COMBINED`, or `NOT_APPLICABLE_WITH_REASON`. A remaining `MISSING` responsibility prevents `PASS` at that affected scope even if the model opens, drawings export, or other stages have passed.
