...
- `project-receipt-compiler.v1.schema.json` — request/readback contract for the subordinate non-authority receipt compiler. It reuses `DOMAIN_PROCESS_INSTANCE` / `DOMAIN_STAGE_INSTANCE` facts and existing closure-object schemas; it does not create another process model and cannot auto-grant professional PASS.
