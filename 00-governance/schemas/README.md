...
- `professional-domain-process.v1.schema.json` — subordinate generic contract for a reusable `PROFESSIONAL_DOMAIN_PROCESS_DEFINITION` or project `DOMAIN_PROCESS_INSTANCE` with embedded `DOMAIN_STAGE_INSTANCE` records. It preserves authentic domain stage IDs, reuses existing Integration maturity semantics, and supports prospective `DECISION_OBJECT_BOUND` stage instances that join decision object → claim → task/claim knowledge mount → required native output → capability roles → owner set / adapter route → artifact → actual readback without creating another professional-process, Capability, Skill or Tool taxonomy.
...
- `project-receipt-compiler.v1.schema.json` — request/readback contract for the subordinate non-authority receipt compiler. It reuses `DOMAIN_PROCESS_INSTANCE` / `DOMAIN_STAGE_INSTANCE` facts and existing closure-object schemas; it does not create another process model and cannot auto-grant professional PASS. Digest fields are paired with explicit `UTF8_TEXT_LF_CANONICAL_V1` or `RAW_BYTES_V1` semantics so Windows CRLF checkout policy cannot change text identity.
