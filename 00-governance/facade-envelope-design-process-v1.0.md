...
- **ASTM E783-02(2018) + ASTM E1105-15(2023 reapproval)**: bounded field methods for installed fenestration/envelope air leakage and water penetration. OLEANDER source mount: Notion `SRC-ASTM-ENVELOPE-FIELD-AIR-WATER-001` (`3e2b86be-5c47-8134-97dd-d16d1b3cd777`). These methods bind actual installed specimen/configuration, apparatus/test pressure/state, observation/result and limitations; they do not define universal project acceptance pressures or sample rates.
...
For installed field air/water evidence, additionally preserve:

`TEST OBJECT / LOCATION → INSTALLED ASSEMBLY + ADJACENT-CONSTRUCTION SCOPE → METHOD / VERSION → APPARATUS / CHAMBER / SPRAY CONFIGURATION → SPECIFIED PRESSURE / CYCLE / STATE → ENVIRONMENTAL CONDITION → EXTRANEOUS-LEAKAGE CONTROL → OBSERVED AIR/WATER PATH → RESULT → DEFECT / REPAIR → RETEST → TRANSFER LIMIT`.

- E783 air-leakage evidence must distinguish the declared specimen leakage from adjacent/perimeter/extraneous paths unless the adapted scope explicitly includes them.
- E1105 field water evidence is installation-sensitive; laboratory/mock-up performance cannot silently stand in for installed-field performance.
- A field sample supports only the actually tested location/configuration/pressure/state and current acceptance basis. Sampling frequency and test pressure remain project/source/authority dependent.
- If a failed field test leads to sealant/gasket/flashing/drainage/installation repair, the pre-repair result is preserved and the affected claim remains open until the applicable retest/readback closes it.
...
- field tests with exact sample/location, installed configuration, method/version, pressure/state, apparatus, observed path/result and declared acceptance owner;
- extraneous/perimeter leakage or hidden-path limitations where material to interpretation;
- defects, repair and retest with old/new configuration identity preserved;
