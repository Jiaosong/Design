...
- `CONSTRAINED_TRANSFORMATION_EXTENSION.md` — existing/baseline objects whose components, relations or states are known and where alternatives should be generated by bounded structural transformation (remove, duplicate/differentiate, split/resequence, reassign role, create/break dependency) without reopening unrelated design variables; requires protected-invariant attacks, matched artifact deltas and Existing Project Repair regression where applicable.
...
- `REPRESENTATION_REDRAW_EXECUTION_EXTENSION.md` — source-bound maps, GIS figures, plans, sections, axons, analytical cuts, node drawings, experience frames, state variants and presentation derivatives that must come from one authority/confidence world through declared redraw operations; style remains claim-dependent rather than a fixed project template.
...
When a mature/bounded baseline already exists and the question is how to generate structurally different repairs or successor candidates without importing unrelated resources, route to `CONSTRAINED_TRANSFORMATION_EXTENSION.md`. Bind the constraint-world map and protected invariants before applying any structural operator; idea quantity is not option-space evidence.
