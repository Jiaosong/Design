...
| Blender Geometry Nodes | Blender Production + `EVD-DCC-BLENDER-GEOMETRY-NODES-5_2-001` | `PRAC-20260830-3D-03`; Preserve/Realize A/B `.blend`, PNG, GLB and receipts recovered from run `33306247098` | `RECOVERED_NATIVE` Blender 5.2; native reopen + explicit evaluated-mesh GLB bake/roundtrip | M6 only for fields, named-attribute domain/type, instance policy, native reopen and export-bake/graph-loss propositions | browser/engine target-runtime import + material/tangent/attribute readback + profiling |
...
| FreeCAD | Modeling Essence + Parametric Design + `EVD-CAD-FREECAD-1_1-20260830-001` | `PRAC-20260830-3D-04`; FreeCAD 1.1.3 `.FCStd`, STEP, parameter sweep and reopen receipts recovered from run `33306247104` | `RECOVERED_NATIVE` FreeCAD 1.1.3 + STEP DCC roundtrip | M6 only for named-parameter/expression dependency, recompute sweep, native reopen and STEP geometry-vs-history-loss propositions | datum/reference-change robustness, Sketcher constraints, assembly/fit; Fusion remains separate HOLD |
...
| Interchange / Technical Art | glTF PBR Interchange + Runtime TA | GN practice now proves Blender→GLB static-geometry bake/graph-loss boundary; Qingjiang project model identity still unrecovered in this pass | `PARTIAL RECOVERED_NATIVE / TARGET RUNTIME HOLD` | bounded interchange semantics practiced; browser/engine parity still not M6 | load recovered GLB in real browser/engine, verify units/axis/material/tangent/attributes and profile |
