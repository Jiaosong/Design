...
| glTF tangent basis / mirrored UV | glTF PBR Interchange + Shading/Texturing | `PRAC-20260830-3D-09`; run `33308860878` | `TARGET_RUNTIME`; Blender 5.2 → GLB → Three r179 | M6 for loop bitangent-sign → glTF `TANGENT.w` → Three tangent sign preservation, mirrored-UV opposite handedness, and normalTexture→normalMap binding. Standard loops remain `+1`; mirrored-U loops remain `-1` end-to-end | numeric TBN/world-normal reconstruction; pixel-level shading parity; negative-scale tangent parity; triangulation-change test |
