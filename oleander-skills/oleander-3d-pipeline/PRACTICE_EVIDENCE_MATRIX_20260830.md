...
|---|---|---|---|---|---|
| Blender polygon/SubD / surface control | Modeling Essence + SubD + Differential Geometry + Representation Bandwidth | `PRAC-20260830-3D-01` Porsche 992 + `PRAC-20260831-3D-11` same-source NURBS/SubD | `RECOVERED_NATIVE` Blender 5.2; Porsche failure study plus shared-source Catmull-Clark `.blend` reopen | M6 only for bounded surface-control and cross-representation propositions; Porsche design remains REJECT | actual Rhino `.3dm` Class-A comparison; Porsche aperture-architecture repair |
| Cross-representation NURBS vs SubD | Modeling Essence + Spline/NURBS Mathematics + DCC Polygon/SubD + Representation Conversion | `PRAC-20260831-3D-11`; run `33376553806` | `RECOVERED_NATIVE` dual worker: FreeCAD 1.1.3/OpenCASCADE B-Spline revolve + Blender 5.2 Catmull-Clark, both native reopen | M6 only for same-source evaluated comparison, controls≠surface points, sampling≠representation, and section/curvature readback. Does not establish Rhino/Class-A parity | Rhino 8 `.3dm` same-source NURBS/SubD + Zebra/EMap/CurvatureGraph/MatchSrf; Houdini remains separate |
| Geometric operator failure anatomy | Geometric Operator Semantics + Modeling Essence + Numerical/Tolerance Robustness | `PRAC-20260831-3D-12`; run `33389249392` | `RECOVERED_NATIVE` FreeCAD 1.1.3 B-Rep + Blender 5.2 modifier sweeps, native save/reopen and cross-operator receipts | M6 only for bounded semantic propositions: kernel exception ≠ invalid return ≠ valid semantic no-op; Clamp/transform state can change realized geometry. **Artifact Review POST-REVIEW FAIL / training not closed** because latest Bevel/Solidify closeups remain too dark | repair visual diagnostic with readable section/thickness gauge; Rhino Offset/Fillet/Shell native parity; arbitrary-topology/self-intersection cases |
| Blender/Cycles material–reflection | Lighting/Radiometry + BSDF + Color + Render Production | `PRAC-20260830-3D-02` XJ01-06B historical + `PRAC-20260830-3D-10` current material-data diagnostic | historical XJ runtime plus current Blender 5.2/Cycles native `.blend` and 9-frame rerun | M6 for controlled diagnostic-lighting and data-map semantics only; Physical CMF HOLD | physical-sample correspondence; HDR/radiometric carrier if required |
| Blender Geometry Nodes | Blender Production + `EVD-DCC-BLENDER-GEOMETRY-NODES-5_2-001` | `PRAC-20260830-3D-03`; Preserve/Realize A/B `.blend`, PNG, GLB and receipts; run `33306247098` | `RECOVERED_NATIVE` Blender 5.2; native reopen + explicit evaluated-mesh GLB bake/roundtrip | M6 only for fields, named-attribute domain/type, instance policy, native reopen and export-bake/graph-loss propositions | production generator with non-synthetic source; hardware/device profiling where material |
| Rhino NURBS/SubD | Rhino Surface Modeling + Rhino 8 tools + Surface Continuity | no recovered `.3dm` benchmark; FreeCAD/OpenCASCADE dual-worker benchmark is only adjacent cross-representation evidence | `DOC_TRANSLATED / NATIVE RUNTIME HOLD` | M4/M5 only; `PRAC-20260831-3D-11` must not be interpreted as Rhino native M6 | actual Rhino `.3dm` CV/span/knot/continuity + Zebra/EMap/CurvatureGraph/MatchSrf + STEP/reopen evidence |
...
| FreeCAD parametric CAD | Modeling Essence + Parametric Design + `EVD-CAD-FREECAD-1_1-20260830-001` | `PRAC-20260830-3D-04` parametric STEP + `PRAC-20260831-3D-11` B-Spline revolve + `PRAC-20260831-3D-12` operator sweep | `RECOVERED_NATIVE` FreeCAD 1.1.3: parametric CAD, OpenCASCADE B-Spline/STEP/native reopen and bounded Fillet/Thickness/Offset failure sweep | M6 only for explicitly tested dependency/roundtrip, B-Spline same-source and operator postcondition/failure propositions; not Fusion/Rhino parity | datum/reference-change robustness, Sketcher constraints, assembly/fit; Rhino/Fusion remain separate HOLD |
...
| Surface-detail representation bandwidth | Texture Map Processing + Surface Detail | `PRAC-20260830-3D-08`; run `33308580990` | `RECOVERED_NATIVE`; macro geometry + meso tangent bake, native reopen, actual preview comparison | M6 for bounded macro/meso representation choice: silhouette IoU `0.9998729`; baked RGB MAE `0.0010110` vs plain `0.0020522` (~50.7% reduction) | displacement/parallax A/B; production texel-density budget; complex seam/cage cases |
| Blender→GLB→browser target runtime | glTF PBR Interchange + Runtime TA | `PRAC-20260830-3D-05`; run `33306786373` | `TARGET_RUNTIME` Chromium + WebGL2 + Three r179; 1 mesh / 1 material / 3980 triangles | M6 for exact baked-GLB identity, real browser load/readback, material existence and axis-extent mapping; not a performance or Design PASS | runtime profiling/device matrix; production asset with textures/instances |
| Signed coordinate / handedness exchange | glTF PBR Interchange + Runtime TA | `PRAC-20260830-3D-07` + 2026-09-01 negative-determinant extension; run `33460924293` | `TARGET_RUNTIME`; four asymmetric signed-axis witnesses plus WebGL framebuffer negative-determinant tangent witness | M6 for tested static signed transform, determinant `+1` handedness preservation, and bounded odd-reflection tangent/fragment contract using `effective_w = tangent.w * sign(det(M_world))`; not full mirror/PBR/hardware parity | non-uniform/nested negative transforms; applied/baked mirror parity; animation/camera/light transforms; hardware/other-engine parity |
| glTF tangent basis / mirrored UV | glTF PBR Interchange + Shading/Texturing | `PRAC-20260830-3D-09`; run `33308860878` plus later numeric/fragment witnesses | `TARGET_RUNTIME`; Blender 5.2 → GLB → Three r179/WebGL2 | M6 for bounded tangent-sign, embedded pixel, numeric TBN and diagnostic framebuffer propositions; not full PBR parity | hardware/other-engine parity; production asset carrier |
...
For cross-representation comparison additionally record:

`ONE SHARED SOURCE HASH → REPRESENTATION A NATIVE MASTER → REPRESENTATION B NATIVE MASTER → EACH EVALUATOR → EVALUATED SECTION/CURVATURE READBACK → SAMPLING SENSITIVITY → REPRESENTATION DELTA → NATIVE REOPEN → ARTIFACT REVIEW → HOLDS`.
...
For geometric operators additionally record:

`INPUT REPRESENTATION → PARAMETER DOMAIN → POLICY / TRANSFORM STATE → OPERATOR → REALIZED GEOMETRY / TOPOLOGY DELTA → KERNEL STATUS → DECLARED POSTCONDITIONS / INVARIANTS → FAILURE SWEEP → NATIVE REOPEN → ARTIFACT REVIEW → HOLDS`.
...
A green workflow is execution evidence, not automatic professional/design approval. For `PRAC-20260831-3D-12`, workflow SUCCESS coexists with **POST-REVIEW FAIL** on the latest visual diagnostic carrier; therefore the training remains open even though bounded semantic propositions are practiced.
