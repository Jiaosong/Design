...
This module does not implement a B-Rep kernel. It serializes solved sketch intent
into a deterministic CAD build request, fingerprints that request, binds an
external sidecar response to a Blender display derivative, and marks the display
stale when upstream CAD intent changes.
...
class CADSidecarContractError(ValueError):
    """Raised when the bounded CAD sidecar contract is malformed or unsafe."""
...
def build_request(
    *,
    ...
    solver: str,
    solver_state: str,
    profile_points_mm: Iterable[Sequence[float]],
    extrusion_depth_mm: float,
    holes: Iterable[dict] = (),
) -> dict:
    """Build a deterministic bounded CAD request from solved sketch intent."""
    ...
    features: list[dict] = [
        {
            "feature_id": f"{ole_id}::CAD-F001",
            "kind": "EXTRUDE",
            "depth_mm": float(extrusion_depth_mm),
        }
    ]
    for index, hole in enumerate(holes, start=2):
        ...
        features.append(
            {
                "feature_id": f"{ole_id}::CAD-F{index:03d}",
                "kind": "THROUGH_HOLE",
                "center_mm": center,
                "radius_mm": radius,
                "through_all": True,
    ...
    return {
        ...
        "source": {
            ...
            "authority": "SOLVED_SKETCH_INTENT",
        },
        "profile": {
            "kind": "POLYLINE",
            "closed": True,
...
def load_response(path: str | Path) -> dict:
    ...
    if response.get("status") != "PASS":
        raise CADSidecarContractError(
            f"CAD sidecar response is not PASS: {response.get('error') or 'unknown error'}"
        )
...
def bind_display_derivative(
    *,
    ...
    """Create/update a Blender mesh that remains a typed CAD display derivative."""
    ...
    if display_payload.get("master_type") != "CAD_NATIVE":
        raise CADSidecarContractError("display payload lost CAD_NATIVE source type")
    if display_payload.get("geometry_authority") != "FREECAD_OCCT_BREP":
        raise CADSidecarContractError("display payload lost FreeCAD/OCCT source authority")
...
def update_stale_state(obj, current_request: dict) -> bool:
    """Mark a bound CAD display stale when upstream request content changes."""
