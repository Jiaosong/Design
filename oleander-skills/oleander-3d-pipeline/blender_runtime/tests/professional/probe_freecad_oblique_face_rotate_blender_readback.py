"""Blender display-only readback for bounded oblique FreeCAD face rotation."""
...
ROOT = Path(os.environ.get("OLEANDER_OBLIQUE_FACE_ROTATE_DIR", "/tmp/oleander-oblique-face-rotate"))
DISPLAY = ROOT / "oleander_oblique_face_rotate_display.json"
MANIFEST = ROOT / "oleander_oblique_face_rotate_manifest.json"
REOPEN = ROOT / "oleander_oblique_face_rotate_readback.blend"
checks: list[str] = []

EXPECTED = {
    "R001": {"yaw": 30.0, "angle": 5.0},
    "R002": {"yaw": -25.0, "angle": -5.0},
}
...
def main() -> None:
    ...
    check(manifest.get("schema") == "OLEANDER_FREECAD_OBLIQUE_FACE_ROTATE_v0.1", "manifest_schema")
    check(manifest.get("status") == "PASS", "manifest_pass")
    check(display.get("schema") == "OLEANDER_OBLIQUE_FACE_ROTATE_DISPLAY_v0.1", "display_schema")
    ...
    for name, expected in EXPECTED.items():
        ...
        rotated_normal = item["expected_rotated_normal"]
        ...
        check(not is_axis_aligned(axis), "oblique_axis_" + name)
        check(close(length(normal), 1.0), "unit_normal_" + name)
        check(close(length(axis), 1.0), "unit_axis_" + name)
        check(abs(dot(normal, axis)) < 1e-6, "axis_tangent_to_face_" + name)
        check(close(length(rotated_normal), 1.0), "unit_rotated_normal_" + name)
        check(manifest[name]["metrics"]["solid_count"] == 1, "one_solid_" + name)
        check(manifest[name]["metrics"]["face_count"] == 6, "six_faces_" + name)

        mesh = bpy.data.meshes.new("OLE_OBLIQUE_FACE_ROTATE_MESH_" + name)
        mesh.from_pydata(item["vertices_mm"], [], item["triangles"])
        mesh.update()
        obj = bpy.data.objects.new("OLE_OBLIQUE_FACE_ROTATE_" + name, mesh)
        ...
        obj["expected_rotated_normal"] = rotated_normal
        obj["units"] = "mm"
        obj["angle_units"] = "deg"
    ...
    for obj in created:
        name = obj.name.removeprefix("OLE_OBLIQUE_FACE_ROTATE_")
    ...
    check(dot(revisions["R001"]["axis_world_direction"], revisions["R002"]["axis_world_direction"]) < 0.99, "world_axis_changes_across_revisions")
    ...
    for name, expected in EXPECTED.items():
        obj = bpy.data.objects.get("OLE_OBLIQUE_FACE_ROTATE_" + name)
        ...
        check(obj["axis_id"] == "AXIS::SELECTED_FACE_CENTER::LOCAL_POS_Y", "reopen_axis_id_" + name)
        check(not is_axis_aligned(list(obj["axis_world_direction"])), "reopen_oblique_axis_" + name)
    ...
    result = {
        "schema": "OLEANDER_OBLIQUE_FACE_ROTATE_BLENDER_READBACK_v0.1",
        "status": "PASS",
        "blender": bpy.app.version_string,
        "revisions": sorted(EXPECTED),
        "checks": checks,
        "authority": {"master": "FreeCAD/OCCT oblique planar face local-axis rotate", "blender": "DISPLAY_DERIVATIVE_ONLY"},
        "non_claims": [
    ...
    print("OLEANDER_OBLIQUE_FACE_ROTATE_BLENDER_READBACK=" + json.dumps(result, sort_keys=True))
