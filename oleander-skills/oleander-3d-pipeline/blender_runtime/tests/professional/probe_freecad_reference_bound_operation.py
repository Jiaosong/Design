...
operation using its center and normal. Width changes, outer-edge filleting, and
one bounded inner-wire topology mutation change the geometry/topology signature,
but the same semantic ref ID binds the correct operation. Ambiguous or missing
resolution returns HOLD with zero mutation.
...
def reference_template():
    return {
        "schema": "OLEANDER_BREP_OPERATION_REFERENCE_v0.2",
        "ref_id": REF_ID,
        "selector": {
...
def main():
    ...
    split_result, op_split = apply_bound_hole(split_amb_source, ref, "R005_SPLIT_TOP_AMBIGUOUS")
    ...
    amb_result, op_amb = apply_bound_hole(amb_source, ref, "R006_COMPOUND_AMBIGUOUS")
    ...
    missing_result, op_missing = apply_bound_hole(missing_source, ref, "R007_MISSING")
    ...
    check(ref["last_good_signature"] == op4["resolved_signature"], "holds_preserve_last_good_signature")
    ...
    for prop, value in [
        ...
        ("OLE_PersistenceBoundary", "NO_SUBSHAPE_ORDINALS;RE_RESOLVE_OR_HOLD;BOUNDED_INNER_WIRE"),
    ]:
    ...
    for name, op in [("R001", op1), ("R002", op2), ("R003", op3), ("R004", op4)]:
    ...
    check(rr.OLE_LastGoodSignature == op4["resolved_signature"], "registry_last_good_reopen")
    check(
    ...
    stage("DISPLAY")
    display = {
        "schema": "OLEANDER_REFERENCE_BOUND_OPERATION_DISPLAY_v0.2",
    ...
    manifest = {
        "schema": "OLEANDER_FREECAD_REFERENCE_BOUND_OPERATION_v0.2",
        "status": "PASS",
        ...
        "reference_contract": {
            ...
            "selector_id": SELECTOR_ID,
            "operation_id": OP_ID,
            "binding": "ref_id -> per-revision semantic resolution -> resolved signature -> actual BRep cut",
            "failure_rule": "ambiguous/missing -> HOLD and zero mutation",
            "ordinal_persistence": "PROHIBITED",
            "bounded_topology_extension": "one planar top face with one pre-existing circular inner wire",
        },
        "revision_operations": [op1, op2, op3, op4, op_split, op_amb, op_missing],
